...
if (isset($_POST['add_doc'])) {
    ...
    $doc_fname = filter_var($_POST['doc_fname'], FILTER_SANITIZE_STRING);
    $doc_lname = filter_var($_POST['doc_lname'], FILTER_SANITIZE_STRING);
    $doc_number = filter_var($_POST['doc_number'], FILTER_SANITIZE_STRING);
    $doc_email = filter_var($_POST['doc_email'], FILTER_SANITIZE_EMAIL);

    // Hash the password securely
    $doc_pwd = password_hash($_POST['doc_pwd'], PASSWORD_BCRYPT);

    // SQL to insert captured values using prepared statements
    $query = "INSERT INTO his_docs (doc_fname, doc_lname, doc_number, doc_email, doc_pwd) VALUES (?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($query);

    if ($stmt) {
        $stmt->bind_param('sssss', $doc_fname, $doc_lname, $doc_number, $doc_email, $doc_pwd);
        $stmt->execute();
