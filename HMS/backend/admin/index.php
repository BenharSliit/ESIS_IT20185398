<?php
    ...
    if(isset($_POST['admin_login']))
    {
        ...
        /*$ad_pwd = password_hash($_POST['ad_pwd'], PASSWORD_BCRYPT);*/
 	...
 	// To check if the session has timed out:
...
<!--?php
    ...
    if (isset($_POST['admin_login'])) {
        $ad_email = $_POST['ad_email'];
        $input_password = $_POST['ad_pwd']; // Get the plain text input password

        // Query the database to fetch the hashed password
        $stmt = $mysqli->prepare("SELECT ad_pwd, ad_id FROM his_admin WHERE ad_email = ?");
        $stmt->bind_param('s', $ad_email);
        $stmt->execute();
        $stmt->bind_result($hashed_password, $ad_id);
        $stmt->fetch();
